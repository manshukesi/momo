...
import pandas as pd
from scipy import stats
def 单变量参数估计(file_path, column_name='average', confidence_level=0.95, freedom=None):
    # 打开数据文件
    df_movies = pd.read_csv(file_path)
    # 计算均值和标准误差
    mean = df_movies[column_name].mean()
    std_error = stats.sem(df_movies[column_name])
    # 设定自由度
    if freedom is None:
        freedom = len(df_movies[column_name]) - 1
    # 计算置信区间
...
单变量参数估计(file_path, 'average', confidence_level=0.95)